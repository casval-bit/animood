...
async function upsertElo(username, field, delta, pointsDelta=0) {
  ...
  if(row) {
    ...
    await sb.query("game_elo", {
      ...
      body: JSON.stringify({ username, elo_chain:400, elo_timeline:400, points_total:0, ...patch }),
    }).catch(()=>{});
  }
}
...
function getAIConfig(diff) {
  return {
    ...
    medium: { failRate: 0.15, delay: 2000 },
    hard:   { failRate: 0.05, delay: 1000 },
  }[diff] || { failRate: 0.15, delay: 2000 };
}
...
export function Matchmaking({ gameType, onMatch, onClose }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      const _eloRow = await sb.query(`game_elo?username=eq.${encodeURIComponent(myUsername)}&limit=1`).catch(()=>[]);
      const elo = _eloRow?.[0] || { elo_chain:400, elo_timeline:400 };
      ...
      const findRoom = async (range) => {
        const twoMinAgo = new Date(Date.now() - 90000).toISOString();
      ...
      // Clean up ALL phantom rooms for this user
      await sb.query(`game_rooms?player1=eq.${encodeURIComponent(myUsername)}&status=eq.waiting&game_type=eq.${gameType}`, {method:"DELETE"}).catch(()=>{});
      await sb.query(`game_rooms?player2=eq.${encodeURIComponent(myUsername)}&status=eq.waiting&game_type=eq.${gameType}`, {method:"DELETE"}).catch(()=>{});
...
export function ChainGame({ room, onClose, onReady }) {
  ...
  const { failRate: aiFailRate, delay: aiDelay } = getAIConfig(room._aiDiff || "medium");
  ...
  useEffect(() => {
    const poll = setInterval(async () => {
      try {
        const rows = await sb.query(`game_rooms?id=eq.${room.id}&select=state,status,winner,updated_at&limit=1`);
        const r = rows?.[0];
        if(!r) return;
        ...
        // Stale detection: opponent hasn't updated in 45s on their turn
        if(r.updated_at && !isMyTurn) {
          const stale = Date.now() - new Date(r.updated_at).getTime() > 45000;
          if(stale) {
            await sb.query(`game_rooms?id=eq.${room.id}`, {method:"PATCH", headers:{...sb.headers,"Prefer":"return=minimal"}, body:JSON.stringify({status:"finished",winner:myUsername})}).catch(()=>{});
            clearInterval(poll);
            setState(s => ({...s, phase:"gameEnd", winner:myUsername, opponentLeft:true}));
            return;
          }
  ...
  const pushState = (newState) => pushStateToRoom(room.id, newState, isAI);
  ...
  const doForfait = async () => {
    if(state.phase !== "gameEnd") {
      if(room.ranked) {
        ...
        await Promise.all([
          upsertElo(myUsername, "elo_chain", -40, 0),
          upsertElo(victim, "elo_chain", 5, 5),
        ]);
      }
      await sb.query(`game_rooms?id=eq.${room.id}`, {
        method: "PATCH",
...
setMsg(t.errInvalidLink);
  ...
  const handleGameEnd = async (winner, scores) => {
    ...
    await Promise.all([
      upsertElo(room.player1, "elo_chain", delta1, p1Won?20:5),
      upsertElo(room.player2, "elo_chain", delta2, p2Won?20:5),
    ]);
    await sb.query(`game_rooms?id=eq.${room.id}`, {
      method: "PATCH",
      headers: { ...sb.headers, "Prefer": "return=minimal" },
...
setMsg(t.errWrongPosition||"âŒ Mauvaise position !");
    ...
    if(gameOver && room.ranked) {
      ...
      await Promise.all([
        upsertElo(room.player1, "elo_timeline", delta1, p1Won?20:isDraw?10:5),
        upsertElo(room.player2, "elo_timeline", delta2, p2Won?20:isDraw?10:5),
      ]);
    }
    await pushState(newState);
    setDragging(null);
  ...
  const doForfaitTL = async () => {
    if(state.phase !== "gameEnd") {
      if(room.ranked) {
        const victim = myUsername === room.player1 ? room.player2 : room.player1;
        await Promise.all([
          upsertElo(myUsername, "elo_timeline", -40, 0),
          upsertElo(victim, "elo_timeline", 5, 5),
        ]);
      }
      await sb.query(`game_rooms?id=eq.${room.id}`, {
        method: "PATCH",
...
export function CluescaleGame({ room, onClose }) {
  ...
  useEffect(() => {
    ...
    if(winner !== myUsername) return; // only winner gets points
    const diff = winScore - secondScore;
    if(diff <= 0) return;
    const earned = diff * 3;
    // Upsert pts_cluescale
    sb.query(`game_elo?username=eq.${encodeURIComponent(myUsername)}&limit=1`)
      .then(rows => {
        const row = rows?.[0];
        const current = row?.pts_cluescale || 0;
        const patch = { pts_cluescale: current + earned, updated_at: new Date().toISOString() };
        if(row) {
...
// Shared: write game state to DB room
async function pushStateToRoom(roomId, newState, isAI=false) {
  if(isAI) return;
  await sb.query(`game_rooms?id=eq.${roomId}`, {
    method: "PATCH",
    headers: { ...sb.headers, "Prefer": "return=minimal" },
    body: JSON.stringify({ state: newState, updated_at: new Date().toISOString() }),
  }).catch(()=>{});
