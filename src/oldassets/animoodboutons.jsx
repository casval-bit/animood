...
async function fetchMoodboardCandidates(selectedMoods, duration, mediaTypes, countries, me) {
  ...
  // Country filter — only apply if not "all" and not just JP selected
  if(countryFilter && !(countryFilter.length===1 && countryFilter[0]==="JP")) {
    const filtered = candidates.filter(a => {
      ...
      const isCN = demo.some(d=>d.includes("donghua"))
        || src.includes("chinese") || src.includes("manhua")
        || allNames.includes("bilibili") || allNames.includes("tencent")
        || allNames.includes("iqiyi");
      const isKR = src.includes("manhwa")
        || demo.some(d=>d.includes("manhwa"));
      const isJP  = !isCN && !isKR;
    ...
    // Only apply if filter returns results — otherwise show everything
