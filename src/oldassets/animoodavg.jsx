...
const sb = {
  ...
  // mood_pts — read from avg table, write votes to mood_pts
  async getMoodPts(mal_id) {
    // Read from avg table first (most accurate — average of both models)
    try {
      const rows = await this.query(`mood_pts_avg?mal_id=eq.${mal_id}&limit=1`);
      if(rows?.[0]) return rows[0];
    } catch {}
    // Fallback to mood_pts if not in avg yet
    const rows = await this.query(`mood_pts?mal_id=eq.${mal_id}&limit=1`);
    return rows?.[0] || null;
  },
  async upsertMoodPts(mal_id, pts, totalVotes) {
    // Write to mood_pts (user votes enrich this table)
  ...
  async upsertMoodPtsAvg(mal_id, pts) {
    // Update avg table when user votes (re-average on the fly)
    return this.query("mood_pts_avg?on_conflict=mal_id", {
      method:"POST",
      headers:{...this.headers,"Prefer":"resolution=merge-duplicates"},
...
async function addUserVote(username, mal_id, newMoods, currentPts) {
  ...
  try {
    await Promise.all([
      ...
      sb.upsertMoodPtsAvg(mal_id, base),
...
async function getPtsForAnime(anime) {
  ...
  // 2. Supabase avg table (average of both models — most accurate)
  try {
...
async function fetchMoodboardCandidates(selectedMoods, duration, mediaTypes, countries, me) {
  ...
  // No need for post-fetch country filter — already applied during Supabase fetch above
  ...
  // Filter: each selected mood must be >= 15% of the anime's total pts
  const MIN_MOOD_PCT = 15;
  const matched = withPts.filter(a => {
    const pct = ptsToPct(a._pts || {});
    return selectedMoods.every(m => (pct[m] || 0) >= MIN_MOOD_PCT);
  });

  // If strict filter returns too few results, relax to 10%
  const pool = matched.length >= 3 ? matched : withPts.filter(a => {
    const pct = ptsToPct(a._pts || {});
    return selectedMoods.every(m => (pct[m] || 0) >= 10);
  });
