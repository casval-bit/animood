...
async function fetchMoodboardCandidates(selectedMoods, duration, mediaTypes, countries, me) {
  ...
  // Thresholds based on number of selected moods
  const MOOD_THRESHOLD = selectedMoods.length === 1 ? 30
    : selectedMoods.length === 2 ? 45 : 55;

  // Excluded: completed + watching + dropped (not watchlist/onhold)
  ...
  candidates = candidates.filter(a => {
    ...
    if(seenTitles.has(root)) {
      if((a.score||0) > (seenTitles.get(root).score||0)) seenTitles.set(root, a);
      return false;
    }
  ...
  // Filter by mood threshold (progressive relaxation)
  ...
  // Score: mood match + MAL score + rank bonus
