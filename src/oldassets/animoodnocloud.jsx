...
// ─── AUTH CONFIG ──────────────────────────────────────────────────────────────
// To change password: edit the value below
const ACCOUNTS = {
  brice: { password: "123456789", name: "Brice", avatar: "🎮" },
};

const AUTH_KEY    = "animood_session";
const PROFILE_KEY = "animood_profile_brice";
...
const jikan = {
  ...
  getAnime:         (id) => jikanCall(`anime-${id}`,   `${JIKAN_BASE}/anime/${id}/full`),
  getStaff:         (id) => jikanCall(`staff-${id}`,   `${JIKAN_BASE}/anime/${id}/staff`),
  getCharacters:    (id) => jikanCall(`chars-${id}`,   `${JIKAN_BASE}/anime/${id}/characters`),
...
// Prefetch anime details in background (call when showing a list)
...
// ─── POINTS STORE ─────────────────────────────────────────────────────────────
// Each anime has a points object: { feelgood: 24, dark: 18, ... }
// Claude seeds with pct values 1:1 as points
// Users add: 1 mood=6pts, 2 moods=3pts each, 3 moods=2pts each
const PTS_STORE_KEY     = "animood_mood_pts";
const PTS_STORE_VERSION = 1;
const PTS_VERSION_KEY   = "animood_mood_pts_version";

function loadPtsStore() {
  try {
    const v = parseInt(localStorage.getItem(PTS_VERSION_KEY)||"0");
    if(v < PTS_STORE_VERSION) {
      localStorage.removeItem(PTS_STORE_KEY);
      localStorage.setItem(PTS_VERSION_KEY, String(PTS_STORE_VERSION));
      return {};
    }
    return JSON.parse(localStorage.getItem(PTS_STORE_KEY)||"{}");
  } catch { return {}; }
}
function savePtsStore(s) {
  try {
    localStorage.setItem(PTS_STORE_KEY, JSON.stringify(s));
    localStorage.setItem(PTS_VERSION_KEY, String(PTS_STORE_VERSION));
  } catch {}
}

let ptsStore = loadPtsStore();

// Convert Claude % output → seed points (1:1)
function pctToPoints(pct) {
  const KEYS = ["feelgood","emotional","hype","dark","chill","twisted","comedy","wholesome","epic"];
  const pts = {};
  KEYS.forEach(k => { pts[k] = Math.round(pct[k] || 0); });
  return pts;
}

// Add user mood votes — idempotent (removes old vote first, then adds new)
function addUserVote(mal_id, newMoods, currentPts) {
  const prev = ptsStore[`${mal_id}_vote`] || { moods:[], pts:{} };
  ...
  const addedPts = {};
  newMoods.forEach(m => {
    addedPts[m] = pointsPerMood;
    base[m] = (base[m]||0) + pointsPerMood;
  });
  ptsStore[mal_id] = base;
  ptsStore[`${mal_id}_vote`] = { moods: newMoods, pts: addedPts };
  savePtsStore(ptsStore);
  return base;
}
...
async function getPtsForAnime(anime) {
  ...
  // Tag with Claude → convert % to points
  const pct = await claudeTagPct(anime);
  const pts = pctToPoints(pct);
  ptsStore[id] = pts;
  savePtsStore(ptsStore);
  return pts;
}
...
// For display: pct of total per mood
function ptsToPct(pts) {
  const total = Object.values(pts).reduce((a,b)=>a+b, 0) || 1;
  const out = {};
  Object.entries(pts).forEach(([k,v]) => { out[k] = Math.round(v/total*100); });
  return out;
}
...
function MoodOctagon({pts}) {
  const KEYS = ["feelgood","emotional","hype","dark","chill","twisted","comedy","wholesome","epic"];
  const size = 300, center = 150, maxR = 90, levels = [25,50,75,100];
  const MOOD_META = {
    feelgood:{emoji:"✨",color:"#FFD93D"}, emotional:{emoji:"💔",color:"#A78BFA"},
    hype:{emoji:"⚡",color:"#F97316"},     dark:{emoji:"🩸",color:"#EF4444"},
    chill:{emoji:"🌿",color:"#34D399"},    twisted:{emoji:"🌀",color:"#06B6D4"},
    comedy:{emoji:"😂",color:"#FCD34D"},   wholesome:{emoji:"🌸",color:"#F9A8D4"},
    epic:{emoji:"⚔️",color:"#FB923C"},
  };

  // Convert pts to pct for display on octagon
  const pct = ptsToPct(pts || {});
  const total = Object.values(pts||{}).reduce((a,b)=>a+b,0);
  ...
  const ptsList = KEYS.map((k,i) => {
    ...
    const r = (v/100)*maxR;
    return {
      key:k, v, rawPts: pts?.[k]||0,
      x: center + Math.cos(angle)*r,
      y: center + Math.sin(angle)*r,
      lx: center + Math.cos(angle)*(maxR+42),
      ly: center + Math.sin(angle)*(maxR+42),
      meta: MOOD_META[k] || {emoji:"?",color:"#818cf8"},
    };
  });

  const polygon = ptsList.map(p=>`${p.x},${p.y}`).join(" ");
  const dominant = Object.entries(pts||{}).sort((a,b)=>b[1]-a[1])[0]?.[0] || "epic";
  ...
  return (
    <div style={{background:"rgba(255,255,255,0.03)",borderRadius:"16px",border:"1px solid rgba(255,255,255,0.08)",padding:"14px",marginBottom:"14px"}}>
      ...
      {total>0&&<div style={{fontSize:"9px",color:"#4b5563",textAlign:"center",marginBottom:"8px"}}>{total} pts · {Object.keys(pts||{}).filter(k=>(pts[k]||0)>0).length} moods</div>}
      <svg viewBox={`0 0 ${size} ${size}`} style={{width:"100%",maxWidth:"300px",display:"block",margin:"0 auto"}}>
        ...
        {KEYS.map((_,i) => {
          const a=(Math.PI*2*i/KEYS.length)-Math.PI/2;
          return <line key={i} x1={center} y1={center} x2={center+Math.cos(a)*maxR} y2={center+Math.sin(a)*maxR} stroke="rgba(255,255,255,0.07)" strokeWidth="1"/>;
        })}
        {/* Shape — no dots */}
        <polygon points={polygon} fill={`${fillColor}28`} stroke={fillColor} strokeWidth="2.5" strokeLinejoin="round"/>
        {/* Emoji labels only — no name, no % */}
        {ptsList.map(p=>(
          <g key={p.key}>
...
const MOOD_TO_GENRES = {
  hype:["1,17,31"], dark:["14,37"], twisted:["7,40,38"],
  emotional:["8,22"], chill:["36,46"], wholesome:["22,36"],
  comedy:["4"], feelgood:["4,30"], epic:["2,10,24"],
};
...
// ─── CONSTANTS ────────────────────────────────────────────────────────────────
const MOODS = [
  {id:"feelgood", label:"Feel Good",    emoji:"✨",color:"#FFD93D"},
  {id:"emotional",label:"Emotional",    emoji:"💔",color:"#A78BFA"},
  {id:"hype",     label:"Hype",         emoji:"⚡",color:"#F97316"},
  {id:"dark",     label:"Dark & Gritty",emoji:"🩸",color:"#EF4444"},
  {id:"chill",    label:"Chill",        emoji:"🌿",color:"#34D399"},
  {id:"twisted",  label:"Twisted",      emoji:"🌀",color:"#06B6D4"},
  {id:"comedy",   label:"Comedy",       emoji:"😂",color:"#FCD34D"},
  {id:"wholesome",label:"Wholesome",    emoji:"🌸",color:"#F9A8D4"},
  {id:"epic",     label:"Epic",         emoji:"⚔️",color:"#FB923C"},
];
...
async function fetchMoodboardCandidates(selectedMoods, duration, mediaTypes, countries, me) {
  ...
  // ── Fetch a broad pool from Jikan — no genre filter, let PCT handle mood matching
  const params = {
    order_by: "score", sort: "desc",
    limit: 25, min_score: isNiche ? 5.5 : 6.5,
    sfw: false,
  };
  if(!mediaTypes.includes("all")) params.type = mediaTypes[0];
  ...
  for(const page of pagesToFetch) {
    try {
      const data = await jikan.searchAnime({...params, page});
  ...
  if(countryFilter) {
    const filtered = candidates.filter(a => {
      ...
      const allNames = [...(a.studios||[]),...(a.producers||[])].map(s=>(s.name||"").toLowerCase()).join(" ");
      const isCN = demo.some(d=>d.includes("donghua"))||src.includes("chinese")||src.includes("manhua")
        ||allNames.includes("bilibili")||allNames.includes("tencent")||allNames.includes("iqiyi");
      const isKR = src.includes("manhwa")||demo.some(d=>d.includes("manhwa"))||allNames.includes("korean");
      const isWestern = !isCN && !isKR && (
        allNames.includes("powerhouse")||allNames.includes("studio mir")||
        ["cartoon","disney","warner","triggerfish"].some(w=>allNames.includes(w))
      );
      const isJP = !isCN && !isKR && !isWestern;
      if(countryFilter.includes("JP")    && isJP)        return true;
      if(countryFilter.includes("CN")    && isCN)        return true;
      if(countryFilter.includes("KR")    && isKR)        return true;
      if(countryFilter.includes("other") && isWestern)   return true;
      return false;
    });
    // Don't empty results if country filter kills everything
  ...
  // Score: sum of selected mood percentages (ALL must contribute)
  // An anime matches if EACH selected mood has at least some percentage
  const scored = withPts.map(a => {
    const pct = ptsToPct(a._pts || {}); const moodScore = selectedMoods.reduce((acc,m) => acc + (pct[m]||0), 0);
    return { ...a, _score: moodScore * 2 + (a.score||7) * 3 };
  });

  // Sort by combined score — naturally surfaces anime strong in ALL selected moods
          ...
          <div style={{display:"flex",gap:"8px",flexWrap:"wrap",marginBottom:"14px"}}>
            {Object.entries(STATUS_COLORS).map(([k,v])=>(
              <div key={k} style={{display:"flex",alignItems:"center",gap:"4px"}}>
                ...
                <span style={{fontSize:"9px",color:"#6b7280",fontWeight:600}}>{v.label}</span>
              </div>
            ))}
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:"9px"}}>
            {[...me.watched].reverse().map(id=>{
...
function AnimeDetail({malId,seedData,onClose}) {
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try {
        ...
        setAnime(ad.data);
        setStaff((sd.data||[]).slice(0,10));
        setCharacters((cd.data||[]).slice(0,8));
        setLoading(false);
  ...
  const save=()=>{
    if(!rating) return;
    // Merge user mood votes into pct store
    if(selMoods.length>0 && animePts) {
      addUserVote(malId, selMoods, animePts);
      setAnimePts(ptsStore[malId]);
    }
...
const COUNTRIES = [
  ...
  {id:"KR",    label:"Coréen",   emoji:"🇰🇷"},
  {id:"other", label:"Autres",   emoji:"🌍"},
];

// Jikan producer country mapping (approximate via source filter)
const COUNTRY_PARAMS = {
  JP: null,           // default — most anime are JP, no extra filter
  CN: "producers",   // handled via custom logic
  KR: "producers",
  other: null,
};
...
export default function AniMoodApp() {
  ...
  const [me, setMe] = useState(()=>{
    const saved = loadProfile("brice");
    return saved || DEFAULT_PROFILE;
  });

  // Persist profile on change
  useEffect(()=>{ if(session) saveProfile("brice", me); },[me]);
