      ...
      <svg viewBox={`0 0 ${size} ${size}`} style={{width:"100%",maxWidth:"300px",display:"block",margin:"0 auto"}}>
        ...
        {levels.map(lvl => {
          const gPts = KEYS.map((_,i) => {
            ...
            return `${center+Math.cos(a)*(lvl/100)*90},${center+Math.sin(a)*(lvl/100)*90}`;
          }).join(" ");
        ...
        {KEYS.map((_,i) => {
          const a=(Math.PI*2*i/KEYS.length)-Math.PI/2;
          return <line key={i} x1={center} y1={center} x2={center+Math.cos(a)*90} y2={center+Math.sin(a)*90} stroke="rgba(255,255,255,0.07)" strokeWidth="1"/>;
        })}
        {/* Shape */}
