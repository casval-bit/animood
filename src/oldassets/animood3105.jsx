...
// ─── MOODBOARD ENGINE ─────────────────────────────────────────────────────────
// MAL demographic IDs for country filtering
const COUNTRY_GENRE_IDS = {
  CN: 47,  // Donghua demographic on MAL
  KR: 43,  // Manhwa demographic on MAL
};

async function fetchMoodboardCandidates(selectedMoods, duration, mediaTypes, countries, me) {
  const countryFilter = countries.includes("all") ? null : countries.filter(c=>c!=="JP");
  ...
  // Base params — no genre filter, let PCT handle mood matching
  const baseParams = {
  ...
  // If CN or KR selected: use Jikan demographic filter (native, accurate)
  // If JP or all: normal search
  const allResults = new Map();
  const pagesToFetch = isNiche ? [1,2,3] : [1,2];

  const queriesToRun = [];
  if(!countryFilter || countryFilter.length === 0 || countries.includes("JP")) {
    // JP or all: standard search
    pagesToFetch.forEach(p => queriesToRun.push({...baseParams, page:p}));
  }
  if(countryFilter?.includes("CN")) {
    pagesToFetch.forEach(p => queriesToRun.push({...baseParams, page:p, genres: String(COUNTRY_GENRE_IDS.CN)}));
  }
  if(countryFilter?.includes("KR")) {
    pagesToFetch.forEach(p => queriesToRun.push({...baseParams, page:p, genres: String(COUNTRY_GENRE_IDS.KR)}));
  }

  for(const params of queriesToRun) {
    try {
