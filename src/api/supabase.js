...
export const sb = {
  ...
  async upsertProfile(data) {
    const row = {
      ...
      active_badge:     data.activeBadge || null,
  ...
  async getReplyCounts(threadIds, excludeUsernames = []) {
    ...
    try {
      const rows = await this.query(`forum_threads?id=in.(${threadIds.join(",")})&select=id,reply_count`) || [];
      const out = {};
      rows.forEach(r => { out[r.id] = r.reply_count || 0; });
      return out;
    } catch { return {}; }
  },
  ...
  try {
    ...
    if(remote) {
      const profile = {
        ...
        activeBadge: remote.active_badge || remote.activeBadge || null,
        banner: remote.banner || null,
