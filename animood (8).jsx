...
const sb = {
  ...
  async upsertMoodPts(mal_id, pts, totalVotes) {
    return this.query("mood_pts?on_conflict=mal_id", {
      method:"POST",
      headers:{...this.headers,"Prefer":"resolution=merge-duplicates"},
      body: JSON.stringify({ mal_id, ...pts, total_votes: totalVotes }),
    });
  },
...
const PTS_CACHE_KEY = "animood_mood_pts";
...
const MOOD_KEYS = ["feelgood","emotional","hype","dark","chill","twisted","comedy","wholesome","epic"];
...
async function getPtsForAnime(anime) {
  ...
  // 3. Tag with Claude → save to both localStorage and Supabase
  const pct  = await claudeTagPct(anime);
  const pts  = pctToPoints(pct);
  ptsStore[id] = pts;
  savePtsCache(ptsStore);
  // Push to Supabase in background
  sb.upsertMoodPts(id, pts, 0).catch(()=>{});
  return pts;
}
